...
if (empty($INSERTED_URL) || $INSERTED_URL == 'undefined') { 
    ...
    $check = $DbCon->query("SELECT S.* FROM MAS_PRODUCT S WHERE S.INSERTED_URL = '$INSERTED_URL' AND PROD_STATUS !='DELETE';");

    if ($check && $check->num_rows > 0) {
        $result = $check->fetch_assoc();
        $resp_status = array_merge(["status" => "ok", "PROD_CODE" => $result['PROD_CODE']], $result);
    } else {
