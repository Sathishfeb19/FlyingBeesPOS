...
if (empty($INSERTED_URL) || $INSERTED_URL == 'undefined') { 
    ...
    $check = $DbCon->query("SELECT S.* FROM MAS_WAREHOUSE S WHERE S.INSERTED_URL = '$INSERTED_URL' AND WH_STATUS !='DELETE';");

    if ($check && $check->num_rows > 0) {
        $result = $check->fetch_assoc();
        $resp_status = array_merge(["status" => "ok", "WH_CODE" => $result['WH_CODE']], $result);
    } else {
